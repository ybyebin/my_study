<template>
  <div class="main-content">
    ...
    <span class="hljs-keyword">var</span> d = { m: <span class="hljs-number">20</span> }</div>
      </pre>
    </div>
    ...
    <p>
      在学习内存空间之前，我们需要对三种数据结构有一个直观的认知。他们分别是
      <strong>堆(heap)，栈(stack)与队列(queue)</strong>。
    </p>
    <div class="session">
      <h5>一、栈数据结构</h5>
    </div>
    ...
    <p>
      ...
      <strong>先进后出，后进先出</strong>的特点。图中已经详细的表明了栈空间的存储原理。
    </p>
    <div class="session">
      <h5>二、堆数据结构</h5>
    </div>
    ...
    <p>
      书虽然也整齐的存放在书架上，但是我们只要知道书的名字，我们就可以很方便的取出我们想要的书，而不用像从乒乓球盒子里取乒乓一样，非得将上面的所有乒乓球拿出来才能取到中间的某一个乒乓球。好比在JSON格式的数据中，我们存储的
      <code>key-value</code>是可以无序的，因为顺序的不同并不影响我们的使用，我们只需要关心书的名字。
    </p>
    <div class="session">
      <h5>三、队列</h5>
    </div>
   ...
   <div class="has-img">
    ...
    <div class="des">
      <p>
        基础数据类型都是一些简单的数据段，JavaScript中有5中基础数据类型，分别是
        <code>Undefined、Null、Boolean、Number、String</code>。基础数据类型都是按值访问，因为我们可以直接操作保存在变量中的实际的值。
      </p>
    </div>
    <blockquote>
    ...
    <div>
      <pre>
            <div class="code">
    <span class="hljs-keyword">var</span> a1 = <span class="hljs-number">0</span>;   <span class="hljs-comment">// 变量对象</span>
    ...
    <span class="hljs-keyword">var</span> c = [<span class="hljs-number">1</span>, <span class="hljs-number">2</span>, <span class="hljs-number">3</span>]; <span class="hljs-comment">// 变量c存在于变量对象中，[1, 2, 3] 作为对象存在于堆内存中</span></div>
      </pre>
    </div>
   <div class="has-img">
    ...
    <div>
      <pre>
            <div class="code">
    <span class="hljs-comment">// demo01.js</span>
    <span class="hljs-keyword">var</span> a = <span class="hljs-number">20</span>;
    <span class="hljs-keyword">var</span> b = a;
    b = <span class="hljs-number">30</span>;

    <span class="hljs-comment">// 这时a的值是多少？</span></div>
      </pre>
    </div>
    ...
    <div>
      <pre>
            <div class="code">
    <span class="hljs-comment">// demo02.js</span>
    <span class="hljs-keyword">var</span> m = { a: <span class="hljs-number">10</span>, b: <span class="hljs-number">20</span> }
    <span class="hljs-keyword">var</span> n = m;
    n.a = <span class="hljs-number">15</span>;

    <span class="hljs-comment">// 这时m.a的值是多少</span></div>
      </pre>
    </div>

    <p>
      在变量对象中的数据发生复制行为时，系统会自动为新的变量分配一个新值。
      <code>var b = a</code>执行之后，a与b虽然值都等于20，但是他们其实已经是相互独立互不影响的值了。具体如图。所以我们修改了b的值以后，a的值并不会发生变化。
    </p>
    ...
    <p>
      在demo02中，我们通过
      <code>var n = m</code>执行一次复制引用类型的操作。引用类型的复制同样也会为新的变量自动分配一个新的值保存在变量对象中，但不同的是，这个新的值，仅仅只是引用类型的一个地址指针。当地址指针相同时，尽管他们相互独立，但是在变量对象中访问到的具体对象实际上是同一个。如图所示。
    </p>
   ...
   <div class="has-img">
    ...
    <div>
      <pre>
            <div class="code">
    <span class="hljs-number">1.</span> 分配你所需要的内存
    <span class="hljs-number">2.</span> 使用分配到的内存（读、写）
    <span class="hljs-number">3.</span> 不需要时将其释放、归还</div>
      </pre>
    </div>
    ...
    <div>
      <pre>
            <div class="code">
    <span class="hljs-keyword">var</span> a = <span class="hljs-number">20</span>;  <span class="hljs-comment">// 在内存中给数值变量分配空间</span>
    alert(a + <span class="hljs-number">100</span>);  <span class="hljs-comment">// 使用内存</span>
    a = <span class="hljs-literal">null</span>; <span class="hljs-comment">// 使用完毕之后，释放内存空间</span></div>
      </pre>
    </div>
    ...
    <p>在JavaScript中，最常用的是通过
      <strong>标记清除</strong>的算法来找到哪些对象是不再继续使用的，因此
      <code>a = null</code>其实仅仅只是做了一个释放引用的操作，让 a 原本对应的值失去引用，脱离执行环境，这个值会在下一次垃圾收集器执行操作时被找到并释放。而在适当的时候解除引用，是为页面获得更好性能的一个重要方式。
    </p>
    <blockquote class="ul-des">
      <ul>
